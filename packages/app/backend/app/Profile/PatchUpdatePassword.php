...
class PatchUpdatePassword implements Pipe
{
    ...
    public function handle(Request $request, Response $response, array $args, callable $next): Response
    {
         ...
         * @see ProfileInput::validatePassword()
         */
        ...
        if ($errors) {
            return $response->withStatus(Response::HTTP_UNPROCESSABLE_ENTITY)
                ->withJson($this->presenter->validationError('Could not update password.', $errors));
        }

        $result = $this->handler->handle($input);
        if (!$result?->success) {
            $message = $result?->errors[0] ?? 'Could not update password.';
            return $response->withStatus(Response::HTTP_UNPROCESSABLE_ENTITY)
                ->withJson($this->presenter->invariantError($message));
        }

        return $response->withStatus(Response::HTTP_OK)
            ->withJson($this->presenter->success('Password updated successfully.'));
    }
}
